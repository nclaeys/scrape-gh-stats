...
echo -e "\n=== Views (last 14 days) ==="
VIEWS=$(gh api --header "Authorization: Bearer ${{ secrets.PAT_TOKEN}}" repos/$REPO/traffic/views | jq -r '.views[] | "\(.timestamp),\(.count),\(.uniques)"')
echo "$VIEWS" | awk -F, '{printf "%s,%s,%s\n",$1,$2,$3}'
if $EXPORT_CSV; then append_csv "$OUTDIR/views.csv" "timestamp,count,uniques" "$VIEWS"; fi

echo -e "\n=== Clones (last 14 days) ==="
CLONES=$(gh api --header "Authorization: Bearer ${{ secrets.PAT_TOKEN}}" repos/$REPO/traffic/clones | jq -r '.clones[] | "\(.timestamp),\(.count),\(.uniques)"')
...
PATHS=$(gh api --header "Authorization: Bearer ${{ secrets.PAT_TOKEN}}" repos/$REPO/traffic/popular/paths | jq -r '.[] | "\(.path),\(.count),\(.uniques)"')
...
REFERRERS=$(gh api --header "Authorization: Bearer ${{ secrets.PAT_TOKEN}}" repos/$REPO/traffic/popular/referrers | jq -r '.[] | "\(.referrer),\(.count),\(.uniques)"')
